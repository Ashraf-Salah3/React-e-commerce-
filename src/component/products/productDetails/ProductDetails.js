    ...
    const fetchProductById = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const coverImage = { imageUrl: product.imageCover };
          const updatedImages = [coverImage, ...(product.images || [])];
            ...
            <div className={`${styles["photo-slider"]}`} {...swipeHandlers}>
              <div className={styles["image-container"]}>
                {isLoading ? (
                  ...
                  <img
                    src={
                      details?.images[currentIndex]?.imageUrl ||
                      details.imageCover
                    }
                    alt="Details"
                  />
            ...
            <div className={styles.price}>
              {localStorage.getItem("authToken") ? (
                <div className={styles["product-price"]}>
                  ...
                  {details?.tradeDiscount !== null && (
                    <p className={styles["sale-price"]}>
                      {Number(details.price - details?.tradeDiscount)}{" "}
                      ج.م
                    </p>
                  )}
                </div>
                ...
                <div className={styles["product-price"]}>
                  ...
                  {details?.saleDiscount !== null && (
                    <p className={styles["sale-price"]}>
                      {Number(details.price - details?.saleDiscount)}
                      ج.م
                    </p>
                  )}
                </div>
              ...
              {!details.isPriceSame && (
                <span>ملاحظة: السعر يختلف حسب الحجم</span>
              )}
            </div>
